...
func TestInjectSiteTitle(t *testing.T) {
	t.Run("replaces_title_with_site_name", func(t *testing.T) {
		html := []byte(`<html><head><title>VeloRoute - AI Routing Gateway</title></head><body></body></html>`)
		settingsJSON := []byte(`{"site_name":"MyCustomSite"}`)

		result := injectSiteTitle(html, settingsJSON)

		assert.Contains(t, string(result), "<title>MyCustomSite - AI Routing Gateway</title>")
		assert.NotContains(t, string(result), "VeloRoute")
	})
	...
	t.Run("returns_unchanged_when_title_has_attributes", func(t *testing.T) {
		...
		html := []byte(`<html><head><title lang="en">VeloRoute</title></head><body></body></html>`)
	...
	t.Run("escapes_ampersand_in_site_name", func(t *testing.T) {
		html := []byte(`<html><head><title>VeloRoute</title></head><body></body></html>`)
		settingsJSON := []byte(`{"site_name":"A&B"}`)

		result := injectSiteTitle(html, settingsJSON)

		assert.Contains(t, string(result), "<title>A&amp;B - AI Routing Gateway</title>")
	})

	t.Run("preserves_rest_of_html", func(t *testing.T) {
		html := []byte(`<html><head><meta charset="UTF-8"><title>VeloRoute</title><script src="app.js"></script></head><body><div id="app"></div></body></html>`)
		...
		assert.Contains(t, string(result), "<title>TestSite - AI Routing Gateway</title>")
	})
}
